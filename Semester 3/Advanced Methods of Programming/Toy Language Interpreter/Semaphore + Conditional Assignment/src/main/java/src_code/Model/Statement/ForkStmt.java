...
public class ForkStmt implements Stmt{
    ...
    public PrgState execute(PrgState state) throws MyException {
        PrgState pr = new PrgState(new MyStack<>(new Stack<>()), state.getSymTable().deepCopy(), state.getOut(), state.getFileTable(), state.getMemHeap(),state.getSemaphoreTable(), stmt);
        return pr;
    }
