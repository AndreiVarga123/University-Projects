...
public class SceneController2 {
    ...
    @FXML
    private TableColumn<Pair<Integer,Integer>,Integer> threadIdColumn;
    private Stmt stmt;
    private PrgState pr;
    private IRepo repo;
    ...
    public void initScene()
    {
        ...
        list.add(pr);
        repo = new Repo(list, "log1"+index+".txt");
        ...
        indexColumn.setCellValueFactory(new PropertyValueFactory<Pair<Integer,Integer>,Integer>("first"));
        threadIdColumn.setCellValueFactory(new PropertyValueFactory<Pair<Integer,Integer>,Integer>("second"));
    ...
    public void populateLockTable(){
        if(!ctrl.getPrgList().isEmpty()) {
            ...
            for (Map.Entry<Integer, Integer> r : ctrl.getPrgList().get(0).getLockTable().getSet()) {
                Pair<Integer, Integer> pair = new Pair<>(r.getKey(), r.getValue());
                obsList.add(pair);
            }
            lockTable.setItems(obsList);
            lockTable.refresh();
        }
    }
