...
{
    std::string error;
    const std::regex pattern(R"(((http|https)://)(www.)?[a-zA-Z0-9@:%._\+~#?&//=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%._\+~#?&//=]*))");
    if(!std::regex_match(link,pattern))
        error+="Invalid link\n";
    return error;
}

void Validator::validate_add(const Repo &r, std::string &title, std::string &release_year, std::string &likes,std::string &trailer)
{
    std::string error;
...
void
Validator::validate_update(const Repo &r, std::string &title, std::string &new_release_year, std::string &new_likes,std::string &new_trailer)
{
    std::string error;
    error+= validate_number(new_release_year,"release year")+ validate_number(new_likes,"likes")+ validate_link(new_trailer);
    std::vector<Movie> arr=r.get_movie_list();
    bool movie_exists=false;
    for (auto & movie : arr)
        if(movie.get_title()==title){
...
void Validator::validate_add_to_watchlist(const Watchlist &ur, std::string title)
{
    std::vector<Movie> arr=ur.get_watch_list();
    for (auto & movie : arr)
        if(movie.get_title()==title)
            throw Validation_exception("Movie already in watchlist\n");
}

void Validator::validate_remove_from_watchlist(const Watchlist &ur, std::string title)
{
    std::vector<Movie> arr=ur.get_watch_list();
    bool movie_exists=false;
    for (auto & movie : arr)
    ...
    if(!movie_exists)
        throw Validation_exception("Movie not in watchlist\n");
}
